...
public class RegionFacadeREST extends AbstractFacade<Region> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") BigDecimal id, Region entity) {
        super.edit(entity);
    }
    ...
    public Region find(@PathParam("id") int id) {
    ...
    public List<Region> findAll() {
        StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("seleccionarRegion");
        StoredProcedureQuery storedProcedure = spq.setParameter("p_ID_REGION", 0);
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Region> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
